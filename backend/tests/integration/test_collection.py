...
@pytest.mark.asyncio
async def test_get_empty_collection(client, create_test_collection, get_auth_header_for_user):
...
@pytest.mark.asyncio
async def test_get_collection(client, create_test_collection, get_auth_header_for_user, create_test_task):
    response = await client.get(
        "collection/get",
        headers={"Authorization":get_auth_header_for_user}
    )
    ...
    assert response.status_code == status.HTTP_200_OK
    print(response.json())
    assert response.json() == [{'id': 1, 'name': 'test_collection', 'Number of Tasks': 1}]
...
@pytest.mark.asyncio
async def test_update_collection_unauthorized_user(client, get_auth_header_for_second_user, create_test_collection:Collection):
    response = await client.patch('collection/', headers = {"Authorization":get_auth_header_for_second_user},
                                  json={"name":"new name",
                                        "id":create_test_collection.id})
    assert response.status_code == status.HTTP_401_UNAUTHORIZED
    assert response.json() == {'detail': 'Unauthorized'}
